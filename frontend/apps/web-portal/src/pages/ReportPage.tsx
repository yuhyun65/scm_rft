...
const recentJobs = [
  ...
  { name: '품질실적_202603.pdf', type: '품질 실적', status: 'IN_PROGRESS', time: '03-17 15:01' },
];
  ...
  return (
    <div className="page-body">
      ...
      <div className="grid-2">
          ...
          <div className="card-body">
            <div className="form-group mb-12">
              ...
              <select style={{ width: '100%' }} value={reportType} onChange={(event) => setReportType(event.target.value)}>
                ...
                <option>거래처별 품질 실적</option>
                <option>품질 검사 결과표</option>
                <option>월간 발주 통계</option>
              </select>
            </div>
            ...
            <div className="form-group mb-12">
              ...
              <select style={{ width: '100%' }}>
                <option>전체</option>
                <option>공급부품(주)</option>
                <option>대성산업(주)</option>
              </select>
            </div>
            <div className="form-group mb-12">
              <label>출력 형식</label>
              <div className="flex gap-12 mt-4">
                {['PDF', 'Excel', 'CSV'].map((item) => (
                  <label key={item} style={{ display: 'flex', alignItems: 'center', gap: 4, fontWeight: 400, fontSize: 12.5 }}>
                    <input type="radio" name="fmt" value={item} checked={format === item} onChange={() => setFormat(item)} /> {item}
                  </label>
                ))}
              </div>
            </div>
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-title">최근 생성 이력</span>
          </div>
          <div className="card-body" style={{ padding: '10px 16px' }}>
            <table>
              ...
              <tbody>
                {recentJobs.map((job) => (
                  <tr key={job.name}>
                    <td className="fs-12">{job.name}</td>
                    <td className="fs-12">{job.type}</td>
                    <td><StatusBadge status={job.status} /></td>
                    <td className="fs-11">{job.time}</td>
                    <td>
                      {job.status === 'COMPLETED' ? (
                        <button className="btn btn-sm btn-outline">다운로드</button>
                      ) : (
                        <button className="btn btn-sm btn-gray">대기 중</button>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      <div className="card">
        <div className="card-header">
          <span className="card-title">최근 3개월 주요 통계 요약</span>
        </div>
        <div className="card-body">
