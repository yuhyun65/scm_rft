...
export default function Header({ onLogout }: Props) {
  ...
  const initial = memberName ? memberName.charAt(0) : '?';

  return (
    <header className="app-header">
      <span className="logo">🔧 HISCM</span>
      {label && <span className="breadcrumb">/ {label}</span>}
      <div className="header-spacer" />
      <div className="user-info">
        <div className="user-avatar">{initial}</div>
        <span>{memberName || '사용자'}</span>
        <button className="btn btn-sm btn-gray" style={{ marginLeft: 8 }} onClick={onLogout}>로그아웃</button>
      </div>
    </header>
  );
}
